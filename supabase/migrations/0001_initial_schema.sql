-- ============================================================================
-- SUPABASE SCHEMA MIGRATION - LEARNHOUSE TO STUFFNTHINGS LMS
...
-- Organizations: Users can view orgs they belong to + public ones
CREATE POLICY "Users can view accessible organizations" ON organizations
    FOR SELECT USING (
        explore = true OR 
        id IN (SELECT org_id FROM user_organizations WHERE user_id = (
...
CREATE POLICY "Public and member courses visible" ON courses
    FOR SELECT USING (
        public = true OR 
        org_id IN (SELECT org_id FROM user_organizations WHERE user_id = (
            SELECT id FROM users WHERE clerk_user_id = auth.jwt() ->> 'sub'
        ))
    );
...
('stuffnthings', 'Stuffnthings Learning Platform', 'hello@stuffnthings.com', 'Git-based learning platform with progressive access')
